...
def _find_account_store() -> AccountStore:
    for value in vars(web_module).values():
        if isinstance(value, AccountStore):
            return value
    raise RuntimeError("未找到 AccountStore 实例")
